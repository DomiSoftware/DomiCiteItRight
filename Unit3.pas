...
begin
  {$IF DEFINED(mswindows)}
    dfn := 'Database='+TPath.Combine(ExtractFilePath(ParamStr(0)), 'cite.sqlite');
  {$ENDIF}
  {$IF DEFINED(iOS) or DEFINED(ANDROID)}
    dfn := 'Database='+TPath.Combine(TPath.GetDocumentsPath, 'cite.sqlite');
  {$ENDIF}
  DTA.Params.Clear;
  DTA.Params.Add(dfn);
  DTA.Params.Add('DriverID=SQLite');
